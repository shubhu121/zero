// media0 entitlements — canonical source: browser-based-tools/MONETIZATION.md. Copy, don't drift.
// Offline license verification. No network calls, ever. DORMANT: nothing in media0 imports
// gate()/getEntitlements() today (everything is free, no /pro page) — this is plumbing so a
// paid feature can be added later by wrapping it in gate() without a refactor.

const HUB = 'media0'; // ← per hub: ai0 | media0 | privacy0 | dev0 | web0 | data0
...
// Returns an ArrayBuffer-backed view (not ArrayBufferLike) so WebCrypto's
// BufferSource params type-check under TS 5.7+ generic typed arrays.
