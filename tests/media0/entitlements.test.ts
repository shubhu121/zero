...
import { verifyLicenseKey } from '../../src/hubs/media0/lib/entitlements';
...
const base = () => ({
  v: 1, email: 'pro@media0.dev', plan: 'pro',
  hubs: ['media0'], iat: Math.floor(Date.now() / 1000), exp: null as number | null,
});

describe('verifyLicenseKey — offline ECDSA P-256 (HUB=media0)', () => {
  it('accepts a valid, correctly-signed key scoped to this hub', async () => {
    ...
    expect(payload?.email).toBe('pro@media0.dev');
